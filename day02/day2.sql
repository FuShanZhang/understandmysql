...
-- 查找名称为“apple”的水果的价格
select f_price from fruits where f_name like 'apple';
-- 查询价格小于10的水果的名称
...
select * from employee e where e.e_salary = (select max(e_salary) from employee inside where inside.dept_no = e.dept_no);
-- 在employee表中，计算每个部门各有多少名员工。
select count(*),dept_no  from employee group by dept_no;
-- 在employee表中，计算不同类型职工的总工资数。
select sum(e_salary) sum from employee group by e_job;
...
select * from employee where year(hiredate) <= year(now()) - 10;(不准确)
...
elect s_id  from fruits f1 where (select count(*) from fruits f2 where f2.s_id = f1.s_id)>1 group by s_id;
...
select o_num,quantity*item_price sum_price from orderitems where quantity*item_price > 100;
-- 将以上内容使用ORDER BY关键字按总订单价格排序显示结果
select o_num,quantity*item_price sum_price from orderitems where quantity*item_price > 100 order by quantity*item_price;
-- 显示fruits表查询结果的前4行
...
select count(*) from orderitems group  by o_num;
...
select f1.* from fruits f1 where f1.f_price = (select max(f2.f_price) from fruits f2 where f1.s_id = f2.s_id);
-- 在fruits表中查找f_name的最大值
select f_name from fruits order by f_name desc limit 1;
-- 在fruits表中查找市场上价格最低的水果
select * from fruits where f_price = (select min(f_price) from fruits);
-- 在fruits表中查找不同供应商提供的价格最低的水果
select f1.* from fruits f1 where f1.f_price = (select min(f2.f_price) from fruits f2 where f2.s_id = f1.s_id);
